...
<b>for file in *.html ; do mv $file `echo $file | sed 's/\(.*\.\)html/\1php/'` ; done</b>
    ...
    <p>Visitor <a href="https://www.itoph.com/">Chris Hancock / itoph.com</a> writes in with a question, and answers it
        himself</p>
...
Any ideas?
    ...
    <p>To which I reply: Chris, I have no idea. The command line still manages to mystify me utterly. Congrats on
        finding the solution to your problem so well! Perhaps your insight will help others!
    </p>

    <p>Reader Michael Dinsmore adds:</p>

    <blockquote>
        I found your page "https://artlung.com/lab/other/unix-batch-file-rename/" when I searched on "batch rename";
        found it useful.
        <br><br>
    ...
    <blockquote>
        ...
        <a href="https://lab.artlung.com/other/unix-batch-file-rename/">https://lab.artlung.com/other/unix-batch-file-rename/</a>
...
<b>for f in *htm ; do mv $f `basename $f htm`html; done"</b>
    ...
    <p>Thanks Kurt! Here's a link for <a
                href="https://www.gnu.org/software/coreutils/manual/html_chapter/coreutils_18.html#SEC110">basename</a>
        from <a href="https://www.gnu.org/">gnu.org</a>.</p>
...
mv \"$i\" \"$i.mp3\"
        ...
        <p>
            It's to do with how the shell does its processing before it passes the args to the command. In the first
            example, the shell is parses the occurances of <code>\"</code> before giving them to <code>echo</code>,
            which gives this (which is actually the easy way to do this):
        </p>
...
mv "$i" "$i.mp3"
        ...
        which then gets parsed again by the shell for the | sh to finally pass the command these args (where <code>&lt;filename&gt;</code>
        is the expansion of <code>$i</code>):
...
"&lt;filename&gt;" "&lt;filename&gt;.mp3"
        ...
        <p>
            ...
            <br/>
            <br/>
            It's easily overlooked, but once you understand how the shell is parsing things before they get to the
            command (ie, it parses <code>*</code> into a list of files in the current directory), a lot of things will
            fall into place.
        </p>
    </blockquote>
    ...
    <a href="https://www.tomkelshaw.com">Tom Kelshaw</a> wrote in April 6, 2009:

    <blockquote>
        I came across it via Google search for batch renaming files. My requirement was actually to remove garbage/octal
        chars that had appeared via bad Input file text. Your command suggestions were excellent, but didn't answer my
        question, whereas after about 4 more hours, this did: <a
                href="https://lists.xiph.org/pipermail/vorbis/2007-August/026974.html">https://lists.xiph.org/pipermail/vorbis/2007-August/026974.html</a>
        Potentially you could add a P.S linking to this listserv post about how to remove garbage chars from filenames?
